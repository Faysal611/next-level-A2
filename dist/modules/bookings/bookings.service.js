...
const createBooking = async (payload) => {
    if (new Date(payload.rent_end_date).getTime() <= new Date(payload.rent_start_date).getTime()) {
        throw new Error("Invalid rent date");
    }
    const date1 = new Date(payload.rent_start_date).setHours(0, 0, 0, 0);
    const date2 = new Date(payload.rent_end_date).setHours(0, 0, 0, 0);
...
const updateBooking = async (bookingId, role, payload) => {
...
const getAllBookings = async (role, userId) => {
    if (role === "admin") {
